...
// added for demo purposes to accept self-signed certificates
  ...
  return baseArgv
    ...
    .options({
      ...
      cert: {
        ...
        required: true,
        default: './certs/IDP_IT_nonRepudiation.crt',
      ...
      key: {
        ...
        required: true,
        default: './certs/IDP_IT_nonRepudiation.key',
      ...
      issuer: {
        ...
        required: true,
        alias: 'iss',
        default: 'https://localhost:7000/idp/'
      },
      acsUrl: {
      ...
      sloUrl: {
        ...
        required: false,
        alias: 'slo',
        default: 'https://localhost:8888/slo'
      },
      audience: {
        description: 'SP Audience URI',
        required: true,
        alias: 'aud',
        default: 'saml-test-sp'
      },
      serviceProviderId: {
      ...
      httpsPrivateKey: {
        ...
        required: true,
        string: true,
        default: './certs/IDP_IT_authentication.key'
      },
      httpsCert: {
        description: 'Web Server TLS/SSL Certificate (pem)',
        required: true,
        string: true,
        default: './certs/IDP_IT_authentication.crt'
      },
      https: {
...
function _runServer(argv) {
  ...
  const httpServer = argv.https ?
    https.createServer(
      { key: fs.readFileSync(argv.httpsPrivateKey),
        cert: fs.readFileSync(argv.httpsCert),
        requestCert: true,
        rejectUnauthorized: false,
        ca: [ fs.readFileSync(argv.caCertificate) ]
      }, app) :
  ...
  const showUser = function (req, res, next) {
    ...
    if (req.client.authorized) {
      ...
      if(Date.parse(cert.valid_to) > Date.now()){
        // check if the certificate is not revocated
        fetch('https://localhost:9000/verify', { method: 'POST', body: JSON.stringify(cert), headers: { 'Content-Type': 'application/json' } })
        .catch(error => {
          console.log(error);
          res.render('error', {
            message: error
        ...
        .then(json => {
          if(json.status == "good") {
            // prepare attributes
            nameId = Math.random() + "";
            nameId = nameId.substring(nameId.indexOf('.') + 1);
            fiscalCode = cert.subject.CN.substring(0, cert.subject.CN.indexOf('\/'));
            const MESI = { A: '01', B: '02', C: '03', D: '04', E: '05', H: '06', L: '07', M: '08', P: '09', R: '10', S: '11', T: '12' };
            let [ anno, giorno ] = [ fiscalCode.substring(6,8), fiscalCode.substring(9,11) ];
          	if (giorno>40) {
          		giorno -= 40;
          		giorno = "0" + giorno;
          	}
          	dateOfBirth = (anno < 20 ? "20" : "19" ) + anno + "-" + MESI[fiscalCode.charAt(8)] + "-" + giorno;
            console.log(cert);
            user = {
              nameId: nameId,
              name: cert.subject.GN,
              familyName: cert.subject.SN,
            ...
            res.render('revoked-cert', {authnRequest: req.authnRequest});
          }
        });
      } else {
        res.render('expired-cert', {authnRequest: req.authnRequest});
      }
  	} else {
      res.render('no-cert-error', {authnRequest: req.authnRequest});
    }
